...
int main() {
    ...
    while (running) {
                ...
                switch (e.key.keysym.sym) {
                    ...
                    case SDLK_RIGHT: {
                        ...
                        if (menuItems[selectedMenuItem] == "Time Format") {
                            ...
                            if (settings.uiScale == "small") settings.uiScale = "medium";
                            else if (settings.uiScale == "medium") settings.uiScale = "large";
                            else settings.uiScale = "small";
                            ...
                            if (it != settings.availableFonts.end()) {
                                size_t index = std::distance(settings.availableFonts.begin(), it);
                                settings.selectedFont = settings.availableFonts[(index + 1) % settings.availableFonts.size()];
                            } else if (!settings.availableFonts.empty()) {
                                // Fallback to first font if current font is not available
                                settings.selectedFont = settings.availableFonts[0];
                            }
        ...
        if (settings.uiScale != lastUiScale) {
            ...
            fontLarge = TTF_OpenFont("assets/font.ttf", largeFontSize);
            fontSmall = TTF_OpenFont("assets/font.ttf", smallFontSize);
